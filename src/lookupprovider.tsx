...
const createGenericLookupWithParam = (
    ...
    return (param): LookupDescriptor => {
        return {
            type: 'lookup',
            store: {
                listFunc: () => api.selectListForLookupWithParam(token(), lookupId, param),
                byIdFunc: (id) => api.selectByIdForLookupWithParam(token(), lookupId, param)(id),
            } as LookupStoreDescriptor,
            valueMember: valueMember,
            displayMember: displayMember,
        };
...
const createGenericPickvalueLookup = (
    ...
    return {
        ...
        store: {
            ...
            byIdFunc: (id) => api.selectByValueForEntityAndField(token(), entity, field)(id),
        } as LookupStoreDescriptor,
        valueMember: valueMember,
        displayMember: displayMember,
    } as LookupDescriptor;
...
const createGenericAutocomplete = (token: () => string, api: MetaLookupApi, lookupId: string): LookupDescriptor => {
    return {
        type: 'autocomplete',
        store: {
...
const createGenericAutocompleteWithParam = (
    ...
    return (param): LookupDescriptor => {
        return {
            type: 'autocomplete',
            store: {
                listFunc: () => api.autoCompleteForLookupWithParam(token(), lookupId, param),
            } as AutocompleteStoreDescriptor,
        };
    };
};
...
const createGenericAllowedStateLookup = (
    ...
    return (param) => {
        return {
            ...
            store: {
                listFunc: () =>
                    api.selectListAllowedForEntityAndIds(token(), entity, Array.isArray(param) ? param : [param]),
                byIdFunc: (id) => api.selectByStateForEntity(token(), entity)(id),
            } as LookupStoreDescriptor,
            valueMember: valueMember,
            displayMember: displayMember,
...
const createGenericLookupByIdentifier = (
    ...
    return {
        type: 'lookup',
        store: {
            listFunc: () => api.selectListForLookupIdentifier(token(), lookupIdentifier),
            byIdFunc: (id) => api.selectByIdForLookupIdentifier(token(), lookupIdentifier)(id),
        } as LookupStoreDescriptor,
        valueMember: valueMember,
        displayMember: displayMember,
    } as LookupDescriptor;
...
export const LookupProvider = ({ children }: LookupProviderProps): JSX.Element => {
    ...
    const [lookups, setLookups] = useState<
        Record<string, LookupDescriptor | LookupCreator | AutocompleteCreator | Array<unknown>> | undefined
    >();
    const {
    ...
    const createLookups = useCallback(
        (requests: Array<LookupRequest>) => {
            if (
                ...
                setRequestedLookups(['userLookup', 'roleLookup', ...requests.map((l) => l.identifier)]);
    ...
    useEffect(() => {
        if (
            ...
            setValue((previousValue) => {
                return { ...previousValue, createLookups, getGenericLookupByIdentifier };
            });
        }
    }, [
        identityUserApiFactory,
